...
def update_sliders(week):
    ...
    ], style={"marginBottom": "10px"}) for crit in week_criteria.get(week, [])]
...
def update_scores(submit_clicks, view_participant, participant, reviewer, week, values, ids):
    ...
    if trig == "submit-score":
        ...
        if sh:
            ...
            df_week = df_week[cols].fillna("")
            sheet_name = week.replace(" ","").lower()
            try:
                target_ws = sh.worksheet(sheet_name)
            except WorksheetNotFound:
                target_ws = sh.add_worksheet(title=sheet_name,
                                             rows=str(len(df_week)+1),
                                             cols=str(len(df_week.columns)))
            target_ws.clear()
            target_ws.update([df_week.columns.tolist()]+df_week.values.tolist())
    ...
    if not df.empty:
        ...
        tf.add_trace(go.Scatter(x=part_df['Week'],y=part_df['Final Score'],mode='lines+markers',name=view_participant))
    tf.update_layout(title=f"{view_participant} Score Trend",xaxis_title="Week",yaxis_title="Score (0–10)",plot_bgcolor='white')
    tf.update_xaxes(showgrid=True,gridwidth=1,gridcolor='lightgrey',minor=dict(showgrid=True,gridwidth=0.5,gridcolor='lightgrey'))
    tf.update_yaxes(showgrid=True,gridwidth=1,gridcolor='lightgrey',minor=dict(showgrid=True,gridwidth=0.5,gridcolor='lightgrey'))
